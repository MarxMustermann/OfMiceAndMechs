...
That should usually be around 10-20 ticks."""
    ...
    def assignQuest(self, extraInfo):
        ...
        if "groupType" not in extraInfo:
            ...
            submenue = src.menues.menuMap["SelectionMenu"]("what group to assign the quest to?",options,targetParamName="groupType")
        ...
        if "questType" not in extraInfo:
            ...
            submenue = src.menues.menuMap["SelectionMenu"]("what quest to assign?",options,targetParamName="questType")
            character.macroState["submenue"] = submenue
            character.macroState["submenue"].followUp = {"container":self,"method":"assignQuest","params":extraInfo}
            return
        ...
        if "amount" not in extraInfo:
            options = []
            submenue = src.menues.menuMap["InputMenu"]("how many NPCs to assign? (0 for all)",targetParamName="amount")
            character.macroState["submenue"] = submenue
            character.macroState["submenue"].followUp = {"container":self,"method":"assignQuest","params":extraInfo}
            return
    ...
    def showMap(self, character):
        ...
        self.submenue = src.menues.menuMap["MapMenu"](mapContent=mapContent,functionMap=functionMap, extraText=extraText, cursor=character.getBigPosition())
    ...
    def beusefulFromMap2(self, extraInfo):
        ...
        if "numNPCs" not in extraInfo:
            submenue = src.menues.menuMap["InputMenu"]("how many NPCs to send?",targetParamName="numNPCs")
            character.macroState["submenue"] = submenue
