...
class CityPlaner(src.items.Item):
    ...
    def unscheduleRoomFromMap(self,params):
        ...
        params["character"].changed("unscheduled room",params)
    ...
    def showMap(self, character, cursor = None):
        ...
        for forest in terrain.forests:
            if (forest[0],forest[1]) in functionMap:
                del functionMap[(forest[0],forest[1])]
            mapContent[forest[1]][forest[0]] = "ff"

        for room in terrain.rooms:
            if not (len(room.itemsOnFloor) > 13+13+11+11 or room.floorPlan or room.storageSlots or len(room.walkingSpace) > 4 or room.inputSlots or room.buildSites or room.tag):
