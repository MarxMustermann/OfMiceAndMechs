...
class RoomManager(src.items.Item):
    ...
    def actionSelection(self,character):
        options = [("storeItem","add item to storage"),("spawnMaintenanceNpc","spawn maintenance npc")]
    ...
    def addTaskSelection(self,character):
        options = [("add item","add item"),("add manager","add manager"),("add machine","add machine"),("add resource terminal","add resource terminal"),("add machine machine","add machine machine"),("add CityBuilder","add CityBuilder"),("clear all","clear all"),("clear item slot","clear item slot")]
    ...
    def addTask(self):
        if self.tasksType == None:
            ...
            if self.tasksType in ("add manager",):
                options = [("StockpileMetaManager","StockpileMetaManager"),("ArchitectArtwork","ArchitectArtwork"),("RoadManager","RoadManager"),("MiningManager","MiningManager")]
                self.submenue = src.interaction.SelectionMenu("select manager type",options)
                self.character.macroState["submenue"] = self.submenue
                self.character.macroState["submenue"].followUp = self.addTask
                return
    ...
    def relayJobOrder(self,task,context):
        ...
        newTasks = [
                {"task":"go to item","command":self.generatePathFromTo([character.xPosition,character.yPosition],[itemSlot[0],itemSlot[1]-1])},
                {"task":"insert job order","command":"scj"},
                relayedTask,
                {"task":"go back","command":self.generatePathFromTo([itemSlot[0],itemSlot[1]-1],[character.xPosition,character.yPosition])},
                ]
        jobOrder.tasks.extend(list(reversed(newTasks)))
...
"""%(self.itemPositions,self.dependencies,self.itemSlotUsage,)
