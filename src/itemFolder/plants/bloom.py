...
class Bloom(src.items.Item):
                ...
                for x in range(0,15):
                    for y in range(0,15):
                        if x < 14 and x > 0 and y < 14 and y > 0:
                            continue
                        chars = []
                        for i in range(0,int(random.random()*2)+3):
                        ...
                        self.container.addAnimation((base_x+x,base_y+y,0),"charsequence",2,{"chars":chars})
    ...
    def localSpawn(self):
        ...
        if not self.dead:
            ...
            if not self.container.moistureMap[position] > 0:
                return
