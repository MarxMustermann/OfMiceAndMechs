    ...
    def apply(self,character):
        if isinstance(character,src.characters.Monster):
            if character.phase == 1:
                character.macroState["commandKeyQueue"] = [("9",[]),("9",[]),("j",[])] + character.macroState["commandKeyQueue"]
...
class AutoTutor(Item):
    ...
    def challengeRun2(self):
        ...
        # copy command
            ...
            if not self.checkInInventory(src.items.SickBloom):
            ...
            if not self.checkInInventory(src.items.Coal):
            ...
            if not self.checkInInventory(src.items.FireCrystals):
