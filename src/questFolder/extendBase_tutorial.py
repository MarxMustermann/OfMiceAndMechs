        ...
        if not self.subQuests:
            ...
            if len(character.getTerrain().rooms) < 2:
                ...
                for candidate in candidates:
                    ...
                    items = terrain.getItemByPosition((candidate[0]*15+7,candidate[1]*15+7))
                    for item in items:
                        if item.type == "RoomBuilder":
                            foundRoombuilder = True
                            break
                ...
                if not foundRoombuilder:
                    quest = src.quests.questMap["FetchItems"](toCollect="RoomBuilder", amount=1, returnToTile=False, takeAnyUnbolted=True, reason="""start off with something simple.
Rooms are build using a RoomBuilder, so fetch one to prepare for building your first room""")
                    return ([quest],None)
            ...
            if len(character.getTerrain().rooms) < 3:
                ...
                if ( not room.getItemByPosition((5,2,0)) or
                    ...
                    quest = src.quests.questMap["PlaceItem"](targetPositionBig=(7,7,0),targetPosition=(5,2,0),itemType="CityPlaner",reason="be able to use it to manage room purposes",boltDown=True)
                    if not dryRun:
            ...
            if len(character.getTerrain().rooms) < 4:
                ...
                if not dryRun and not self.shownTutorialStep1:
                    text = """

Hello there.

A quick interupption from me, the developer of this game, MarxMusterman.
You are kind of playing the tutorial for the basebuilder aspect of this game right now.


You have seen the absolute basics for now:
* moving and interacting with items
* the quest system
* getting epoch rewards (spawning NPCs)
* drawing stockpiles
* designating rooms

If you are not enjoying yourself or just don't care about tutorials,
you can start to ignore the tutorials instructions.
The tutorial will likely break, but you can have fun playing around.


If you have fun, press enter to continue running the game.
It will guide you through more NPC activity by building more and more rooms.
I will interrupt you again at a later point in time.

May your enemies be swift and deadly,
MarxMusterman
"""
                    src.interaction.showInterruptText(text)
                    self.shownTutorialStep1 = True
            ...
            if len(character.getTerrain().rooms) < 6:
                ...
                """
                for room in terrain.rooms:
                    for inputSlot in room.getEmptyInputslots(fullyEmpty=True):
                        if not inputSlot[1]:
                            continue
            ...
            if len(character.getTerrain().rooms) < 6:
                for room in terrain.rooms:
                    inputSlots = room.getEmptyInputslots(itemType="Scrap",fullyEmpty=True)
            ...
            if hasAllNPCs:
                ...
                if numItems >= 100:
                    if not hasNPC and epochArtwork.charges >= 10:
                        quest = src.quests.questMap["GetEpochReward"](rewardType="spawn "+random.choice(["resource fetching","hauling","resource gathering","machine operating"])+" NPC",reason="spawn another clone to help you out")
                        return ([quest],None)
