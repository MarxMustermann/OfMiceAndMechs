...
class GetEpochReward(src.quests.MetaQuestSequence):
    ...
    def generateTextDescription(self):
        ...
        if self.reason:
            reason = f",\nto {self.reason}"
        text = [f"""
""",(src.pseudoUrwid.AttrSpec(src.interaction.highlighted_ui_color,"black"),"Claim a reward"),f""" for the work you have done{reason}.

You accumulated some mana.
Spend it to claim a magical effect.
"""]
...
""")
            case _:
                text.append(f"""
Claim the reward: {self.rewardType}
""")
