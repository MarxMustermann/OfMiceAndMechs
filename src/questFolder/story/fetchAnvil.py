...
class FetchAnvil(src.quests.MetaQuestSequence):
    type = "FetchAnvil"

    def __init__(self, description="fetch anvil", creator=None, targetPositionBig=None, reason=None, story=None):
        questList = []
...
Use the k or K keys to pick up items."""
    ...
    def getNextStep(self,character=None,ignoreCommands=False, dryRun = True):
        ...
        # go to loot
        items = self.getAnvil(character)
        random.shuffle(items)
        for item in items:
            quest = src.quests.questMap["GoToPosition"](targetPosition=item.getPosition(),ignoreEndBlocked=True,reason="be able to pick up the Anvil")
            return ([quest],None)

        # dummy return
        return self._solver_trigger_fail(dryRun,"unknown reason")

    def getAnvil(self,character):
        ...
        if not renderForTile:
            if isinstance(character.container,src.rooms.Room):
                if not character.getNearbyEnemies():
                    for item in self.getAnvil(character):
