...
class FetchPainter(src.quests.MetaQuestSequence):
    type = "FetchPainter"

    def __init__(self, description="fetch painter", creator=None, targetPositionBig=None, reason=None, story=None):
        questList = []
...
A Painter looks like this: """,sample_painter.metaRender()]
        return text
    ...
    def getNextStep(self,character=None,ignoreCommands=False, dryRun = True):
        ...
        # go to loot
        items = self.getPainter(character)
        random.shuffle(items)
        for item in items:
            quest = src.quests.questMap["GoToPosition"](targetPosition=item.getPosition(),ignoreEndBlocked=True,reason="be able to pick up the Painter")
            return ([quest],None)

        # dummy return
        return self._solver_trigger_fail(dryRun,"unknown reason")

    def getPainter(self,character):
        ...
        if not renderForTile:
            if isinstance(character.container,src.rooms.Room):
                if not character.getNearbyEnemies():
                    for item in self.getPainter(character):
